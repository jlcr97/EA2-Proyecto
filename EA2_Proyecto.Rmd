...
Correlación considerando edad, gasto_mon y educa_jefe
```{r}
...
Tras reducir el modelo por dos variable; es decir, tras quitar las erogaciones totales y el sexo, el modelo mejorado queda como
```{r}
  mejoraCDMX <- lm(datos$ing_cor ~ edad_jefe  + gasto_mon  + educa_jefe, datos) 
  summary(mejoraCDMX) 
```
Se decidió quitar el sexo y las erogaciones totales porque considerando el modelo con las 5 variables, teníamos una $R^2 = 0.76$ mientras que con el modelo mejorado con las 3 variables restantes, tenemos a $R^2 = 0.74$. Como la diferencia no es muy grande, por el *principio de parsimonia* nos quedamos con el modelo con menos variables.
Se eligieron las 3 variables restantes basándonos en la optimización dada por "ols_step_best_subset(modeloCDMX)"
